 ...
 */
class NoKPrjMgntModelProjectComments extends JModelList {
	private $tableName = '#__nok_pm_project_comments';
	...
	protected function getListQuery() {
		...
		$query
			->select(array('c.id', 'c.title', 'c.published', 'c.createddate', 'c.createdby', 'c.project_id', 'p.title AS project'))
		...
		if (!empty($search)) {
			if (stripos($search, 'id:') === 0) {
				...
				$query->where('(c.title LIKE ' . $search . ' OR p.title LIKE ' . $search . ')');
			}
		}
